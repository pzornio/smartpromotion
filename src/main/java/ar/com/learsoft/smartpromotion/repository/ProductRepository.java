...
public interface ProductRepository extends JpaRepository<Product, Integer>{
	...
	@Query(value = "SELECT p.ID, p.DETAILS, p.NAME, p.PRICE, p.TYPE,(p.PRICE*COUNT_VAL)  AMOUNT_TOTAL  FROM PRODUCT p \r\n"
			+ "JOIN (SELECT COUNT(*) COUNT_VAL, ip.PRODUCTS_ID FROM INVOICE_PRODUCTS ip JOIN (SELECT ID INVOICE_ID FROM INVOICE WHERE CLIENT_ID= :clientId and purchase_date > :timestamp) i  \r\n"
			+ "WHERE i.INVOICE_ID=ip.INVOICE_ID GROUP BY  ip.PRODUCTS_ID ) pc WHERE pc.PRODUCTS_ID  = p.ID AND p.PRICE*COUNT_VAL>:itemAmountTotal", nativeQuery = true)
	public List<Product> findClientProductCountAmount(Integer clientId, Timestamp timestamp,Integer itemAmountTotal);
}
